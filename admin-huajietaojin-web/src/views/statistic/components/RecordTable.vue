<template>
  <div>
    <el-table :data="records" v-loading.body="loading" element-loading-text="Loading" border fit highlight-current-row>
      ...
      <el-table-column  width="120" label="企业ID" align="center">
      ...
      <el-table-column  width="240" label="店铺名称" align="center">
        <template slot-scope="scope">
          {{scope.row.store_id}}/{{scope.row.store_name}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="最终价格" width="120" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  filters: {
    statusFilter(status) {
      return status === true ? "success" : "gray";
    }
  },
  methods: {
